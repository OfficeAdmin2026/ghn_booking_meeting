...
-- ============================================================
-- SELF-HOST TEST DEPLOY: admin account for the MSNV/allowlist login flow.
-- The users/rooms seed above predates that flow and has no employee_id, so
-- without this nobody (not even admin) could log in on a fresh DB. The
-- matching allowed_employees row is inserted separately after the backend
-- creates that table on first boot (see deploy/README.md).
-- ============================================================
INSERT INTO users (id, email, full_name, employee_id, department, role, is_active) VALUES
('99999999-9999-9999-9999-999999999999', 'admin.test@ghn.vn', 'Admin Test', 'ADMIN01', 'IT', 'admin', true);
